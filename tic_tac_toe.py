...
class TicTacToe:
    ...
    def __init__(self, names=None):
        """initialization"""
        if names is None:
            self.players = [input("Введите имя первого игрока: "),
                            input("А теперь второго игрока: ")]
        else:
    ...
    def __str__(self):
        """converting of the board to a string"""
        res = ""
        for i in range(1, 10):
            j = self.board[i-1]
            if j == 0:
    ...
    def make_turn(self, string=None):
        """get the move from console"""
        pos = ""
        if string is None:
            ...
            for pos in string:
                if pos.isdigit() and int(pos) in range(1, 10) and self.board[int(pos)-1] == 0:
                    break
        return int(pos)

    @staticmethod
    def ask_for_repetition(string=None):
        """repetition of the game after it's end"""
        ans = ""
        if string is None:
